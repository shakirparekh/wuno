...
from test_framework.messages import CSigShare, msg_qsigshare
from test_framework.p2p import P2PInterface
from test_framework.test_framework import DashTestFramework
from test_framework.util import assert_equal, assert_raises_rpc_error, force_finish_mnsync, wait_until_helper_internal
...
class LLMQSigningTest(DashTestFramework):
    ...
    def run_test(self):
        ...
        def wait_for_sigs(hasrecsigs, isconflicting1, isconflicting2, timeout):
            wait_until_helper_internal(lambda: check_sigs(hasrecsigs, isconflicting1, isconflicting2), timeout = timeout)

        def assert_sigs_nochange(hasrecsigs, isconflicting1, isconflicting2, timeout):
            assert not wait_until_helper_internal(lambda: not check_sigs(hasrecsigs, isconflicting1, isconflicting2), timeout = timeout, do_assert = False)
