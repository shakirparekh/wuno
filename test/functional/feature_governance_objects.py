...
class wentunoGovernanceTest (DashTestFramework):
    ...
    def run_test(self):
        ...
        p1 = self.prepare_object(object_type, "%064x" % 0, time_start, 0, "SortByTime1", 1)
        p2 = self.prepare_object(object_type, "%064x" % 0, time_start + 10, 1000, "SortByTime2", 10)
        p3 = self.prepare_object(object_type, "%064x" % 0, time_start - 10, 1000000000, "SortByTime3", 20)
        p4 = self.prepare_object(object_type, "%064x" % 0, time_start + 1, -20, "SortByTime4", 400)
        p5 = self.prepare_object(object_type, "%064x" % 0, time_start + 30, 1, "SortByTime5", 100000000)

        rpc_list_prepared = self.nodes[0].gobject_list_prepared()
        ...
        rpc_list_prepared = self.nodes[0].gobject_list_prepared(2)
        ...
        rpc_full_list_pre_restart = self.nodes[0].gobject_list_prepared()
        self.restart_node(0)
        rpc_full_list_post_restart = self.nodes[0].gobject_list_prepared()
        assert_equal(rpc_full_list_pre_restart, rpc_full_list_post_restart)
        # Create more objects so that we have a total of 11
        self.prepare_object(object_type, "%064x" % 0, time_start, 0, "More1", 1)
        self.prepare_object(object_type, "%064x" % 0, time_start, 0, "More2", 1)
        self.prepare_object(object_type, "%064x" % 0, time_start, 0, "More3", 1)
        self.prepare_object(object_type, "%064x" % 0, time_start, 0, "More4", 1)
        # Make sure default count is 10 while there are 11 in total
        assert_equal(len(self.nodes[0].gobject_list_prepared()), 10)
        assert_equal(len(self.nodes[0].gobject_list_prepared(12)), 11)
        # Make sure it returns 0 objects with count=0
        assert_equal(len(self.nodes[0].gobject_list_prepared(0)), 0)
        # And test some invalid count values
        assert_raises_rpc_error(-8, "Negative count", self.nodes[0].gobject_list_prepared, -1)
        assert_raises_rpc_error(-8, "Negative count", self.nodes[0].gobject_list_prepared, -1000)
