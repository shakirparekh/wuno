# CJDNS support in wentuno Core

It is possible to run wentuno Core over CJDNS, an encrypted IPv6 network that
...
done outside of wentuno Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the wentuno network and individual nodes.
...
before it will work with your wentuno Core node. This is described in steps
...
## Run wentuno Core with CJDNS

Once you are connected to the CJDNS network, the following wentuno Core
...
When enabled, this option tells wentuno Core that it is running in an
...
IPv6 local network. This helps wentuno Core perform better address management:
...
CJDNS support was added to wentuno Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `wentuno-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/wentuno/wentuno/blob/master/doc/tor.md) for
details.

## CJDNS-related information in wentuno Core

There are several ways to see your CJDNS address in wentuno Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `wentuno-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `wentuno-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `wentuno-cli help getnodeaddresses` for details.
