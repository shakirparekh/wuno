wentuno Core version 4.2.0.0 is now available from:

  https://github.com/wentuno/wentuno/releases/tag/v4.1.0
...
Upgrade Instructions: https://wentuno.readme.io/v4.1.0/docs/wentuno-41-upgrade-guide
...
BEFORE INSTALLING 4.1.0, PLEASE READ: https://wentuno.readme.io/v4.1.0/docs/wentuno-41-upgrade-guide
...
See: https://wentuno.readme.io/v4.1.0/docs/wentuno-41-upgrade-guide
...
- [Reduce memory](https://github.com/wentuno/wentuno/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running wentuno Core on WUNOtems with
...
wentunocheckmint
- `wentunocheckmint` Useful for those that want to check if a wentuno Transaction
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of wentuno
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `wentunod -help` output for these two options for
...
- In 4.0.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 4.1.0, this flag is now __disabled__ by default. If you want to compile wentuno Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in wentuno Core 0.18.
...
- BIP 61 reject messages were deprecated in v4.0. They are now disabled
  ...
  future version of wentuno Core. (#14054)

- To eliminate well-known denial-of-service vectors in wentuno Core,
  ...
  This prevents wentuno Core from sending the BIP111 NODE_BLOOM service
  ...
  of wentuno Core and nodes that have manually enabled BIP37 support,
...
- By default, wentuno Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `wentuno-cli -getinfo` has been renamed to
...
WUNONEVM BRIDGE
------------------
A first-of-its-kind trust-less permission-less decentralized bridge from wentuno <-> Ethereum. Two way
bridges allow for fractional value ownership across multiple chains. You can transfer your native ERC20 to wentuno SPTs or transfer your wentuno SPT to a new ERC20 established on Ethereum. Over 2 years of research and development went into this feature so many thanks for the count-less hours nights and days spent by security researchers, developers, testers and community volunteers who helped push us create quality code that is well-tested and analyzed for secure usage.  We have spent alot of time finalizing design, re-working under security reviews and making similar assumptions of attack vectors that exist on Bitcoin today in a decentralized environment. We are confident the technology will enable value transfer across chains in a reliable, secure way and is the basis for future research on general trust-less bridge designs. The goal is to enable users benefit from utility of wentuno but in the future we will allow not only Ethereum users from benefiting from wentunos PoW security and ZDAG/Assets Payment channel functionality but more chains to come once we have generalized the solution. Read more about the bridge: https://github.com/wentuno/WUNOethereum-docs

To enable to bridge, we coded 348,000 as the block height where wentuno Mint's will validate (Ethereum to wentuno transactions). Also the ERC20BalanceManager proxy contract address is defined in ChainParams so the consensus code can validate the Ethereum transaction originated on the correct contract. Geth is used to validate Ethereum transactions on wentuno so it is imperative that Geth is synced and running, it will automatically start when wentuno is launched and you may use `getblockchaininfo` to check if Geth is `synced` after a few minutes. It runs in light mode so it only needs to download roughly the last 2 weeks of block headers.

Another application that runs in the background is the [relayer](https://github.com/wentuno/relayer) which takes block information from Geth and feeds it into wentuno using `wentunosetethheaders` RPC function storing into a database. This allows wentuno core to continue operating without making inter-process calls. The functionality is re-org aware in case Ethereum re-orgs the [relayer](https://github.com/wentuno/relayer) will re-send the correct blocks, and the code in `wentunosetethheaders` will ensure the headers of Ethereum are hash-linked (current block -> prev matches previous header).

Ethereum Contracts
------------------
wentunoBattleManager - https://etherscan.io/address/0x637e110A6C61cfd3a8838eC25a2DBF75A0F34B30#code
wentunoClaimManager - https://etherscan.io/address/0x8230De6a088B455B5b83a629B0843746ED1AC5B6#code
wentunoSuperblocks - https://etherscan.io/address/0xD1e95176a81671c7aeBE68135Ba63073Aa3b642A#code
wentunoERC20Manager - https://etherscan.io/address/0x6100c8e90c4295de37f2c1114b03735193484cfd#code
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how wentuno Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  wentuno Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, wentuno Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 wentuno address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  WUNO), the RPC commands `walletcreatefundedpsbt` and
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in wentuno Core v4.0.0), this can eliminate a
...
- #368 Bounty - CheckwentunoMint cutoffHeight attacks
- #328 wentuno-geth.exe won't run if folder (windows) has space.
...
- #15649 Add ChaCha20Poly1305@wentuno AEAD (jonasschnelli)
...
- #15639 wentuno-wallet tool: Drop libwentuno_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in wentunod/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "wentuno-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of wentunod build (NicolasDorier)
...
- #16441 remove qt libjpeg check from wentuno_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_wentuno to auto generated project files (sipsorcery)
...
- #15352 Reduce noise level in test_wentuno output (practicalswift)
...
- #15896 feature_filelock, interface_wentuno_cli: Use PACKAGE_NAME in messages rather than hardcoding wentuno Core (luke-jr)
...
- #16277 Suppress output in test_wentuno for expected errors (gertjaap)
...
- #16489 log: harmonize wentunod logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (wentunoVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example wentuno.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBwentuno_SERVER (Bushstar)
...
- #16080 build/doc: update wentuno_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in wentuno.conf (fanquake)
...
- #16621 add default wentuno.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the wentuno-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, wentuno.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/wentunod.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/wentuno/wentuno/).
