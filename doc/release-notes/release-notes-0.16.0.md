wentuno Core version 0.16.0 is now available from:

  <https://wentunocore.org/bin/wentuno-core-0.16.0/>
...
wentuno Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
wentuno data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile wentuno Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µWUNO" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- wentuno-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- wentuno-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18470 to avoid conflict with testnet's default of 18370.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest wentuno.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.wentuno.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for wentuno-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to wentuno-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µWUNO to bits (jb55)
...
- #7142 `801dd40` Travis: Test build against WUNOtem libs (& Qt4) (luke-jr)
...
- #11621 `1f7695b` [build] Add temp_wentuno_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #12168 `45cf8a0`  #include WUNO/fcntl.h to just fcntl.h (without WUNO/) (jsarenik)
- #12095 `3fa1ab4` Use BDB_LIBS/CFLAGS and pass --disable-replication (fanquake)
- #11711 `6378e5c` wentuno_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.wentuno doesn't exist before build (MeshCollider)
...
- #11144 `cee4fe1` Move local include to before WUNOtem includes (danra)
...
- #11164 `38a54a5` Fix boost headers included as user instead of WUNOtem headers (danra)
- #11143 `3aa60b7` Fix include path for wentuno-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in wentuno-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop wentunod (OmeGak)
...
- #10529 `331352f` Improve wentunod WUNOtemd service file (Flowdalic)
