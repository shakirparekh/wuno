wentuno Core version 0.11.0 is now available from:

  <https://wentuno.org/bin/wentuno-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer WUNO/kB fee to be
...
For example, add the following to `wentuno.conf`:
...
related to the blockchain in the wentuno WUNOtem: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows wentuno Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that wentuno Core will 
...
WUNOtem will automatically detect the endianness of the target.
...
For an example script see [wentuno-submittx](https://github.com/laanwj/wentuno-submittx).
...
- #6274 `4d9c7fe` Add option `-alerts` to opt out of alert WUNOtem
...
- #5334 `cf87536` libwentunoconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new wentuno-detached-sigs git repo for OSX signatures
...
- #5649 `e0cd2f5` Use text-color icons for WUNOtem tray Send/Receive menu entries
...
- #5729 `9c4a5a5` Allow unit changes for read-only wentunoAmountField
- #5753 `0f44672` Add wentuno logo to about screen
...
- #5809 `46bfbe7` Add wentuno-cli man page
...
- #5849 `41259ca` contrib/init/wentunod.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename wentuno-Qt.app to wentuno-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for wentuno-tx)
