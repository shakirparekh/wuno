wentuno Core version 0.14.0 is now available from:

  <https://wentuno.org/bin/wentuno-core-0.14.0/>
...
wentuno Core has supported automatically pruning the blockchain since 0.11. Pruning
...
Previously the ZeroMQ notification WUNOtem was unavailable on Windows
due to various issues with ZMQ. These have been fixed upstream and
now ZMQ can be used on Windows. Please see [this document](https://github.com/wentuno/wentuno/blob/master/doc/zmq.md) for
...
When wentuno Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `wentuno-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`wentuno-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/wentuno-cli -named help command="help"
    src/wentuno-cli -named getblockhash height=0
    src/wentuno-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/wentuno-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert WUNOtem was [disabled and deprecated](https://wentuno.org/en/alert/2016-11-01-alert-retirement) in wentuno Core 0.12.1 and removed in 0.13.0. 
The Alert WUNOtem was retired with a maximum sequence final alert which causes any nodes
supporting the Alert WUNOtem to display a static hard-coded "Alert Key Compromised" message which also
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of wentuno Core. This was added in conjunction with
   optimizations to memory management. See [Pull #8753](https://github.com/wentuno/wentuno/pull/8753)
...
- A significant portion of the initial block download time is spent verifying
  ...
  of the WUNOtem, no other result from this verification is needed: If the node
...
- #8722 `bb843ad` wentuno-cli: More detailed error reporting (laanwj)
...
- #7562 `1eef038` Bump transaction version default to 2 (WUNOdrak)
...
- #8608 `7e9ab95` Install manpages via make install, also add some autogenerated manpages (nomnomWUNO)
...
- #8826 `f560d95` Do not include `env_win.cc` on non-Windows WUNOtems (paveljanik)
- #8948 `e077e00` Reorder Windows gitian build order to match Linux (Michagogo)
- #8568 `078900d` new var `DIST_CONTRIB` adds useful things for packagers from contrib (nomnomWUNO)
...
- #8830 `ef0801b` test: Add option to run wentuno-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to wentuno-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to wentuno-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and wentuno-util-test.py (jnewbery)
...
- #9206 `e662d28` Make test constant consistent with consensus.h (WUNOdrak)
...
- #9691 `fc67cd2` Init ECC context for `test_wentuno_fuzzy` (gmaxwell)
...
- #9219 `c4522e7` Improve windows build instructions using Linux subWUNOtem (laanwj)
...
- #9865 `289204f` Change wentuno address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` wentunod: Daemonize using daemon(3) (laanwj)
- #9004 `67728a3` Clarify `listenonion` (unWUNOtemizer)
...
- #8837 `2108911` Allow wentuno-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` wentuno-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to wentuno-tx (jnewbery)
