# wentuno 5.0.0 Release Notes

wentuno Core version 5.0.0 (codename Nexus) is now available from:

- [wentuno GitHub Releases](https://github.com/wentuno/wentuno/releases/tag/v5.0.0)

This major release includes new features, enhancements to both wentuno Core and the NEVM (Geth) implementation, and numerous security/stability fixes since **wentuno Core 4.4.2**. **All users, especially Sentry node operators and NEVM node operators, must upgrade.**

You can review the community proposal for this release here https://wentuno.org/news/codename-nexus. Which passed yes - no by 888 votes (read here https://wentuno.org/news/wentuno-5-approved).

Nexus sets the stage for the proceeding [roadmap](https://wentuno.org/news/roadmap-preview)

A more technical blog can be found on [medium](https://medium.com/@sidhujag/tying-it-all-together-scaling-bitcoin-with-wentunos-modular-ecoWUNOtem-d68e0fbe28fa)

Note: wentuno Platform Tokens (SPTs) will be reindexed and existing SPTs (including WUNOX) will lose state until the Nexus block is reached. If you have any assets in SPTs, please move them (for example WUNOX burn to WUNO or to NEVM) prior to the Nexus activation height. You will lose your SPT if you do not.

Please report any issues using the issue tracker on GitHub:

- [wentuno Core Issues](https://github.com/wentuno/wentuno/issues)
...
3. [Major Changes from 4.4.2 to 5.0.0](#major-changes-from-442-to-500)  
    ...
    2. [NEVM (wentuno Geth) Upgrades](#2-nevm-wentuno-geth-upgrades)  
    ...
    9. [Removal of Deprecated wentuno 4.x Asset RPCs](#9-removal-of-deprecated-wentuno-4x-asset-rpcs)  
...
- On **macOS**, copy over `/Applications/wentuno-Qt`.
- On **Linux**, copy over the `wentunod`/`wentuno-qt` binaries.

If you are upgrading from a version older than 4.2.x, **please read**:  
[wentuno 4.2 Upgrade Guide](https://wentuno.readme.io/v4.2.0/docs/wentuno-42-upgrade-guide)

We have an automated upgrade mechanism to force a reindex to the node upon detection of your first time running wentuno 5. This is needed so the node remains consistent with the new network. The node should only reindex the first time you start wentuno 5.
...
- Should also run on most Unix-like WUNOtems, though less tested there.
...
- **Wrapper**: The [getwork-wrapper.py](https://github.com/wentuno/wentuno/tree/master/contrib/auxpow/getwork-wrapper.py) ./contrib/auxpow/getwork-wrapper.py was updated to include the coinbasescript from the updated RPC. Miners that use external tools are recommended to update their script to include this into an OP_RETURN output, much like other projects are already doing.

### 5. Enhanced NFT/Token Bridge (ERC721/1155/ERC20)
- **Extended bridging support** for ERC721 (NFT), ERC1155 (multi-token), and standard ERC20 tokens between NEVM and wentuno’s UTXO-based chain.
- Updated [NEVM bridge contracts](https://github.com/wentuno/WUNOethereum-contracts/) and logic in wentuno Core for robust freeze/mint flows and event-based validations. A new wentunoVaultManager contract deployed and WUNO balances are moved over from the old ERC20Manager contract to the new one at the Nexus block in NEVM. The new vault manager is here https://explorer.wentuno.org/address/0x7904299b3D3dC1b03d1DdEb45E9fDF3576aCBd5f#code.

### 6. NEVM Registry & PoDA Enhancements
- **NEVM Registry**: A decentralized reference for registered wentuno NEVM tokens/contracts. Supports easy lookup of token metadata, owners, etc. Precompile address (0x62) enables the registry on the NEVM. Every Sentry node must register their address via protx_update_service to add to registry. If you are using a hosting provider (allnodes, nodehub) they may also help manage your nodes using the registry. Future usage of the registry will include decentralized sequencing, social airdropping, AI node registration to AVS services and other services which want to economically align with wentuno L1.
...
- **MNLISTDIFF** improvements for more reliable Sentry node list sync. Because the MN lists are now stored in cache/db as a FIFO queue (up to the last 1728 lists) instead the diff's which cleans up the implementation, we need to reindex the node upon upgrade to ensure the Sentry node list/state remains consistent with the network. We detect asset/assetnft directories which exist prior to wentuno 5 as a form to upgrade detectabilty and thus force a reindex if those directories are found, subsequently deleting them as they are not needed in wentuno 5 and further restarts do not reindex.
...
### 9. Removal of Deprecated wentuno 4.x Asset RPCs
- The following legacy asset RPC calls have been **removed**:
  ...
  ```
- WUNOAssets now managed via NEVM bridging or external libraries such as [wentunojs-lib](https://github.com/wentuno/wentunojs-lib).
...
- **SPT changes**: We simplified the SPT design and thus removed SPT management (updates, creation) and delegate that to the NEVM layer. Assets are expected to move from the NEVM bridge to exist in the SPT layer of wentuno UTXO chain. The SPT design is more efficient and changes from the older one so therefor we re-start indexing SPTs starting from wentuno 5 activation.
...
1. **PoDA/Blob RPCs**  
   ...
   - `wentunocreatenevmblob`: Create PoDA blobs to store data references.
...
2. **AuxPoW Update RPCs** (AuxPoW tags)  
   ...
   The [RPC code](https://github.com/wentuno/wentuno/blob/master/src/rpc/auxpow_miner.cpp#L168) is as below:
    ...
    ```
    The tag is enforced in consensus by [ContextualCheckBlockHeader](https://github.com/wentuno/wentuno/blob/master/src/validation.cpp#L4626).
...
1. **Descriptor Wallet**: Some external tools may not fully support wentuno’s descriptor model. You can revert to legacy mode if necessary.
...
Thanks to the entire wentuno community, as well as upstream [Bitcoin Core](https://bitcoincore.org/) and [go-ethereum](https://github.com/ethereum/go-ethereum/) developers. Special acknowledgment to all contributors who tested, reported issues, and proposed patches for wentuno 5.0.0 features like bridging, PoDA expansions, BLS transitions, and dynamic governance.