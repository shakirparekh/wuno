# PSBT Howto for wentuno Core

Since wentuno Core 0.17, an RPC interface exists for Partially Signed wentuno
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/wentuno/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for wentuno transactions that are not fully signed
...
#### Multisig with multiple wentuno Core instances
...
wentuno Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `wentuno-cli` is used.
...
Later, when *V* WUNO has been received on *Amulti*, and Bob and Carol want to
