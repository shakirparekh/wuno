# Support for Output Descriptors in wentuno Core

Since wentuno Core v0.17, there is support for Output Descriptors. This is a
...
- P2PKH addresses (base58, of the form `S...` for mainnet or `[nm]...` for testnet). Note that P2PKH addresses in descriptors cannot be used for P2PK outputs (use the `pk` function instead).
...
- Segwit addresses (bech32 and bech32m, of the form `WUNO1...` for mainnet or `tWUNO1...` for testnet, defined in [BIP 173](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki) and [BIP 350](https://github.com/bitcoin/bips/blob/master/bip-0350.mediawiki)).
...
on wentuno's OP_CHECKMULTISIG opcode. To support these, we introduce the
...
the entire path from the master down. [BIP174](https://github.com/wentuno/bips/blob/master/bip-0174.mediawiki) standardizes this by
...
existing wentuno Core wallets, a convenience function `combo` is
...
All RPCs in wentuno Core will include the checksum in their output. Only
