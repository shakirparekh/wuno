...
The data directory is the default location where the wentuno Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.wentuno/`
macOS    | `$HOME/Library/Application Support/wentuno/`
Windows  | `%APPDATA%\wentuno\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `wentuno.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual wentuno blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `wentuno.conf`        | User-defined [configuration settings](wentuno-conf.md) for `wentunod` or `wentuno-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `wentunod.pid`        | Stores the process ID (PID) of `wentunod` or `wentuno-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `wentunod` or `wentuno-qt`; can be specified by `-debuglogfile` option
...
`./`               | `ip_asn.map`          | IP addresses to Autonomous WUNOtem Numbers (ASNs) mapping used for bucketing of the peers; path can be specified with the `-asmap` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [wentuno.conf](wentuno-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`wentuno-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by wentuno Core:
