...
wentuno_CONFIG_ALL="--enable-suppress-external-warnings --disable-dependency-tracking"
...
if [ -n "$ANDROID_TOOLS_URL" ]; then
  ...
  CI_EXEC ./autogen.sh
  CI_EXEC ./configure "$wentuno_CONFIG_ALL" "$wentuno_CONFIG" || ( (CI_EXEC cat config.log) && false)
...
CI_EXEC "${BASE_ROOT_DIR}/configure" --cache-file=config.cache "$wentuno_CONFIG_ALL" "$wentuno_CONFIG" || ( (CI_EXEC cat config.log) && false)

CI_EXEC make distdir VERSION="$HOST"

export P_CI_DIR="${BASE_BUILD_DIR}/wentuno-$HOST"

CI_EXEC ./configure --cache-file=../config.cache "$wentuno_CONFIG_ALL" "$wentuno_CONFIG" || ( (CI_EXEC cat config.log) && false)
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # using the Linux getrandom WUNOcall. Avoid using getrandom by undefining
  # HAVE_WUNO_GETRANDOM. See https://github.com/google/sanitizers/issues/852 for
  # details.
  CI_EXEC 'grep -v HAVE_WUNO_GETRANDOM src/config/wentuno-config.h > src/config/wentuno-config.h.tmp && mv src/config/wentuno-config.h.tmp src/config/wentuno-config.h'
fi
