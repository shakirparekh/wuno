...
if ! wentuno-cli listwallets 2>/dev/null | grep -q "$WALLET_NAME"; then
  echo "Wallet \"$WALLET_NAME\" not found. Creating wallet..."
  # createwallet parameters can differ by wentuno version; this is the basic usage
  wentuno-cli createwallet "$WALLET_NAME"
fi

# Attempt to load the wallet (in case it's newly created or was unloaded)
# Some versions of wentuno automatically load a newly created wallet; if so, this command is harmless.
wentuno-cli loadwallet "$WALLET_NAME" 2>/dev/null

# ---------------------------------------
# 2. Get or generate a new address from that wallet
# ---------------------------------------
echo "Retrieving a new address from wallet \"$WALLET_NAME\"..."
MINING_ADDRESS=$(wentuno-cli -rpcwallet="$WALLET_NAME" getnewaddress)
...
while true; do
  SYNC_STATUS=$(wentuno-cli mnsync status 2>/dev/null)
...
while true; do
  ...
  wentuno-cli -rpcwallet="$WALLET_NAME" generatetoaddress 1 "$MINING_ADDRESS" 1000000000
