...
static UniValue generateBlocks(ChainstateManager& chainman, const CTxMemPool& mempool, const CScript& coinbase_script, int nGenerate, uint64_t nMaxTries)
{
    ...
    while (nGenerate > 0 && !ShutdownRequested()) {
        ...
        if (!GenerateBlock(chainman, pblocktemplate->block, nMaxTries, nExtraNonce, block_out, /*process_new_block=*/true)) {
            break;
        }
...
{
    return RPCHelpMan{"generate", "has been replaced by the -generate cli option. Refer to -help for more information.", {}, {}, RPCExamples{""}, [&](const RPCHelpMan& self, const node::JSONRPCRequest& request) -> UniValue {
        ...
        [&](const RPCHelpMan& self, const node::JSONRPCRequest& request) -> UniValue
{
    ...
    if (!GenerateBlock(chainman, block, max_tries, extra_nonce, block_out, process_new_block) || !block_out) {
...
// NOTE: Unlike wallet RPC (which use wentuno values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
