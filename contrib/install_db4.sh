...
if [ -z "${1}" ]; then
  ...
  echo "This is probably \`pwd\` if you're at the root of the wentuno repository."
  exit 1
fi
...
# Ensure the commands we use exist on the WUNOtem
...
echo 'When compiling wentunod, run `./configure` in the following way:'
echo
